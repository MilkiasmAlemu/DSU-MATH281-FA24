...
# ╔═╡ e7ed2129-7e57-4036-af02-67a465df6e24
@bindname N Slider(1:250,default=100, show_value=true)

# ╔═╡ 5d1afc9e-3649-4c78-a794-cf46714fd76e
@bindname p Slider(0:0.005:1, default=0.5, show_value=true)

# ╔═╡ 0874d104-4eba-4a67-9c9a-c0ed3fa3f65a
X = Binomial(N,p)

# ╔═╡ 51384909-9b53-49cd-92f0-dba87e59d674
@bindname ymax Slider(0:0.01:1, default=0.1, show_value=true)

# ╔═╡ 6bd5d4a8-0d09-4929-991d-f0ad5372238c
begin
    local width = 500
    local height = 300
    ...
    <script>
      ...
      function scaleY(value) {
        // Scale from 0-ymax to minY-maxY
      ...
      function updateLines() {
        ...
        for (let i = 0; i <= N; i++) {
          const x = paddingLeft + ((i + 0.5) * scaleRange / (N + 1));
        ...
        const exX = paddingLeft + ((ex + 0.5) * scaleRange / (N + 1));
        ...
        const exMinusStddevX = paddingLeft + (((ex - stddev) + 0.5) * scaleRange / (N + 1));
        const exPlusStddevX = paddingLeft + (((ex + stddev) + 0.5) * scaleRange / (N + 1));
...
# ╔═╡ e899e2b0-cd0d-4254-843b-7db89cbd2f90
pdf(X, 50)

# ╔═╡ 5ecff5cd-f446-4ce0-b33f-0096ccf80758
StatisticalMeasures = [mean(X), var(X), std(X), skewness(X), 3+kurtosis(X)]

# ╔═╡ 60feb9c6-c336-41de-b450-e2ce43a0e059
@bindname k Slider(0:10, default = 1, show_value=true)

# ╔═╡ e00a7684-ba58-4154-816b-6392a8913f1d
cdf(X, mean(X) + k * std(X)) - cdf(X, mean(X) - k * std(X))
