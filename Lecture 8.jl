...
# ╔═╡ 537e8aac-dc2f-4981-a587-21757b979b4e
using PlutoUI, HypertextLiteral, UUIDs

# ╔═╡ fb6ee027-47b4-454a-b6a7-ef16444da8da
using Distributions

# ╔═╡ 7222cfd8-c2d1-415c-8b84-43996a7922b2
@htl("<h1>Lecture 8</h1>")

# ╔═╡ 85121c75-68c3-47f2-a9cd-1b4ed6bede23
@htl("""
<hr>
...
# ╔═╡ f3b69ea7-ac11-4c53-8b05-e1d8fc6d4e86
@bindname a Slider(range(-5,4.9,step=0.1), show_value=true, default=0)

# ╔═╡ fdd4f842-05b2-4f26-ac57-2034ce1bf917
@bindname b Slider(round.(range(a+0.1,5,step=0.1),digits=1), show_value=true, default=1)

# ╔═╡ bb4885cc-3bae-43da-8173-2b62e83e500a
X_Uniform = Uniform(a,b)

# ╔═╡ 4504a16a-4297-43d7-9802-4a949a1ec0e8
[mean(X_Uniform), var(X_Uniform), std(X_Uniform), skewness(X_Uniform), kurtosis(X_Uniform, false)]

# ╔═╡ e117f7bb-43a1-4b3e-879f-62ac382f286e
begin
	x_slider = @bindname x_range RangeSlider(range(-5,5,step=0.01),show_value=true, default=0:3)
	x_slider
end

# ╔═╡ 282b34d9-f96e-40b2-840c-a1a11b230cc2
cdf(X_Uniform, x_range[end]) - cdf(X_Uniform, x_range[begin])

# ╔═╡ ccf82cbe-c639-4eaa-959e-1b2887d2f99a
@htl("""
<hr>
...
# ╔═╡ 6a982675-ab22-4943-8a91-c3f79353c112
@bindname μ Slider(range(-5,5, step=0.1), show_value=true, default=0)

# ╔═╡ 07ef1db1-4939-45ca-95ba-bc364ad361ab
@bindname σ Slider(range(0.01,5, step=0.01), show_value=true, default=1)

# ╔═╡ 1f9a57f8-3ea4-4695-8950-d7e309302a16
X_Normal = Normal(μ,σ)

# ╔═╡ e25aad8d-3ee1-457e-a9ee-77665951e49b
[mean(X_Normal), var(X_Normal), std(X_Normal), skewness(X_Normal), kurtosis(X_Normal, false)]

# ╔═╡ 2adbbeee-063a-41b8-8ea2-727109c2275d
x_slider

# ╔═╡ 8f0494bc-6c85-4d5a-b625-1101b196666b
cdf(X_Normal, x_range[end]) - cdf(X_Normal, x_range[begin])

# ╔═╡ 8b5c89b5-1be5-4971-a4e0-9ed832b90f22
@htl("""
<hr>
...
# ╔═╡ f9bb74e4-4a61-4a56-a0c9-3fc213636af6
@bindname α Slider(0.1:0.1:5, show_value=true, default=2)

# ╔═╡ b8e4237b-f595-4e4f-bc86-0a0904349f45
@bindname θ Slider(0.1:0.1:2, show_value=true, default=1)

# ╔═╡ 02d9362d-578e-4380-9118-67cc66f2ef66
X_Gamma = Gamma(α,θ)

# ╔═╡ 19477240-efcc-4636-9c99-bf33f4ed9746
[mean(X_Gamma), var(X_Gamma), std(X_Gamma), skewness(X_Gamma), kurtosis(X_Gamma, false)]

# ╔═╡ b9c0cdec-b349-41d5-9f27-834e642ca2e7
x_slider

# ╔═╡ 95c09168-af23-4e9e-b673-87983b939912
cdf(X_Gamma, x_range[end]) - cdf(X_Gamma, x_range[begin])

# ╔═╡ 754edfd4-94bb-41c0-bbc1-7c0d5d90c4ce
# Function for Plotting a Continuous Distribution
function plotContDist(X::UnivariateDistribution)
    local width = 500
    local height = 300
    local bottom_margin = 30
    local x_start = max(min(0, minimum(X)), -10) - 0.5
    local x_end = min(max(0, maximum(X)), 10) + 0.5
    local paddingLeft = 25
    local slider_height = 30
	local circle_radius = 3

    local sample_number = 1000
    local x_vals = range(x_start, x_end, length=sample_number)
    local cdf_vals = cdf.(X, x_vals)
    ...
    <svg width="$(width)" height="$(height)" style="background-color: white;" id=$(DistUUID*"-axis-svg")>
	  ...
	  <g id=$(DistUUID*"-cdf-curve-group")></g>
    ...
    <div style="background-color: grey; opacity: 0.75; width: $(width)px; height: $(slider_height)px; position: relative; display: flex; justify-content: center; align-items: center;">
      <input type="range" id=$(DistUUID*"-slider") min="$(paddingLeft)" max="$(width-paddingLeft)" value="$(paddingLeft)" style="width: calc(100% - 2 * $(paddingLeft)px);"/>
    </div>

    <script>
      const svg = document.getElementById($(DistUUID*"-axis-svg"));
      const slider = document.getElementById($(DistUUID*"-slider"));
	  ...
	  const cdfCurveGroup = document.getElementById($(DistUUID*"-cdf-curve-group"));
      ...
      function updateArea(maxValue) {
        ...
        for (let i = 0; i < x_vals.length && scaleX(x_vals[i]) <= maxValue; i++) {
          points += ` \${scaleX(x_vals[i])},\${scaleY(pdf_vals[i])}`;
        }
        points += ` \${maxValue},\${scaleY(0)}`;
        filledArea.setAttribute("points", points);
        filledArea.setAttribute("fill", "rgba(0,0,255,0.3)");
        areaGroup.appendChild(filledArea);
      }

      function updateCDF(maxValue) {
        while (cdfCurveGroup.firstChild) {
          cdfCurveGroup.removeChild(cdfCurveGroup.firstChild);
        }

        const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
		let endValue = -100;
		let endHeight = 0
        let d = `M \${scaleX(x_vals[0])} \${scaleY(cdf_vals[0])}`;
        for (let i = 1; i < x_vals.length && scaleX(x_vals[i]) <= maxValue; i++) {
          d += ` L \${scaleX(x_vals[i])} \${scaleY(cdf_vals[i])}`;
		  endValue = x_vals[i];
		  endHeight = cdf_vals[i];
        }
        path.setAttribute("d", d);
        path.setAttribute("stroke", "rgba(0,0,0,0.75)");
        path.setAttribute("fill", "none");
        path.setAttribute("stroke-width", 4);
        cdfCurveGroup.appendChild(path);
	
		const percentilePoint = document.createElementNS("http://www.w3.org/2000/svg", "circle");
		percentilePoint.setAttribute("cx", scaleX(endValue));
		percentilePoint.setAttribute("cy", scaleY(endHeight));
		percentilePoint.setAttribute("r", circleRadius);
		percentilePoint.setAttribute("fill", "black");
		percentilePoint.setAttribute("opacity", 0.75);
		cdfCurveGroup.appendChild(percentilePoint);

		const percentileLabel = document.createElementNS("http://www.w3.org/2000/svg", "text");
		percentileLabel.setAttribute("x", scaleX(endValue)+30);
		percentileLabel.setAttribute("y", scaleY(endHeight));
		percentileLabel.setAttribute("fill", "black");
		percentileLabel.setAttribute("font-size", "15px");
		percentileLabel.setAttribute("text-anchor", "middle");
		percentileLabel.textContent = (100 * endHeight).toFixed(2) + "%";
		cdfCurveGroup.appendChild(percentileLabel);
      }
      ...
      slider.addEventListener("input", function(event) {
        ...
        updateArea(sliderValue);
        updateCDF(sliderValue);
      });

      updatePDF();
      updateArea(paddingLeft);
	  updateCDF(paddingLeft);
	  updateLines();
    </script>
    """)
end;

# ╔═╡ 71583ed9-d7cf-4638-8154-aaa0a9843393
plotContDist(X_Uniform)

# ╔═╡ b96e5479-4c4e-42ee-a764-38602d250040
plotContDist(X_Normal)

# ╔═╡ be04724d-74ac-4f29-af35-26c9d84a095f
plotContDist(X_Gamma)
...
manifest_format = "2.0"
project_hash = "e0a427ca87e77952e5e4c200b916a7f8a7b74971"
